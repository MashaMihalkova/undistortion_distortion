...
def pre_compute_waz(pi, height, width, img_coordinate):
    ...
    :param height:
    :param width:
     :param img_coordinate: coordinate information matrix
    :return:
    '''

    # height*width*Number of control points
    wi = np.reciprocal(
    ...
    # height*width*Number of control points*2
    ...
    # height*width*Number of control points*2*1
    z1 = np.repeat(z1.reshape(height, width, 1, 2, 1), [pi.shape[0]], axis=2)

    # height*width*Number of control points*1*2
    ...
    # height*width*Number of control points*2*2
...
def compute_G(img_coordinate, pi, height, width, thre = 0.7):
    ...
    # Computing control area center
    # p_ = (max + min) // 2
    p_ = np.sum(pi, axis=0) // pi.shape[0]

    # Calculation control area
    ...
    k1 = 0.9  # (p_ - [0, 0])[1] / (p_ - [0, 0])[0]
    k2 = 0  # (p_ - [height, 0])[1] / (p_ - [height, 0])[0]
    k4 = 0  # (p_ - [0, width])[1] / (p_ - [0, width])[0]
    k3 = 0  # (p_ - [height, width])[1] / (p_ - [height, width])[0]
    ...
    # If local deformation is not required, the value of cita can be set to 1
    # cita = 1
